	...
	<div class="main">
		<div class="main-header">
			...
			<div class="main-header-search">
				<div class="func-btns">
					<sapi-search v-model="params.keyword" placeholder="按帐号、实名、页面标题、页面路径、IP进行搜索" @on-search="getTableData"></sapi-search>
		...
		<div class="main-table table-no-border width-p100">
			...
			<div class="table-box">
				<el-table v-if="tableData" border :data="tableData" @sort-change="sortChange" :height="maxBodyHeight">
					<el-table-column prop="AccessTime" label="访问时间" show-overflow-tooltip sortable>
						<template slot-scope="props">
							{{$dateFormat("yyyy-MM-dd hh:mm",props.row.AccessTime)}}
						</template>
					</el-table-column>
					...
					<el-table-column label="页面标题" show-overflow-tooltip>
						<template slot-scope="props">
							<a href="javascript:void(0)" @click.stop="openViewDialog(props.row)">{{props.row.Title}}</a>
						</template>
					</el-table-column>
					<el-table-column prop="Source" label="页面路径" show-overflow-tooltip>
					</el-table-column>
	...
	export default {
		...
		methods: {
			getTableData() {
				...
				this.$get("/api/plat/accessLogs/", this.params, function(res) {
			...
			openViewDialog(row) {
				...
				this.option = row;
				this.visible = true;
				this.viewUrl = "/api/plat/accessLogs/";
			},
			getTimeArr() {
				var end = new Date();
				var start = new Date();
		...
		components: {
			"systemlog-view": (resolve) => {
				require(['./access-view.vue'], resolve);
			},
			"sapi-search": search,
			"filter-role": filter
		},
