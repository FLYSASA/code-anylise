...
export default function ZmCanvasCrop(opt, saveCallBack,errorCallBack){
	this.init(opt);
...
ZmCanvasCrop.prototype = {
	...
	readFile: function(){
		...
		if(typeof FileReader==='undefined'){ 
		    ...
		    self.errorCallBack(Vue.__t('avatar.browsersDoNotSupport'));
		    self.fileName="";
		...
		function readFile(){ 
		    ...
		    if(!/image\/\w+/.test(file.type)){ 
		        //alert("文件必须为图片！");
		        self.errorCallBack(Vue.__t('avatar.mustBeImage'));
		        self.fileName="";
		        return false; 
		    } 
		    var isCorrectFile = /jpg$|jpeg$|png$/ig.test(file.name);
		    if(!isCorrectFile) {
				self.errorCallBack(Vue.__t('avatar.formatError'));
				self.fileName="";
				return false;
			}
	...
	drawCavDown: function(src){
	    ...
	    self._img.onload = function(){
	    	...
	    	if(!self.imgTrue()){
	    		...
	    		self.errorCallBack(Vue.__t('avatar.imageSize') + self._option.crop_min_width + '*' + self._option.crop_min_height);
	    		self.fileName="";
	    		return;
	    	} 
	    	//让宽或者高撑满
...
/*					$('.resize-point').off('mousedown');
	    ...
	    function drawDown(){
	    	//console.log(self._img_show.width)
	    	//self._$box.css('marginTop',(314-self._img_show.height)/2);
	...
	save: function(){
		...
		$('.review-image').empty().append($result);
		$result[0].getContext('2d').drawImage(self._img, 
			self._save.left, self._save.top, self._save.width, self._save.height,
			0, 0, self._save.width, self._save.height
		);
		$result.css('transform','scale('+(96/self._save.width)+')');
		var dataUrl = $result[0].toDataURL('image/png', 0.6);
		...
		blobData.name=self.fileName+'.'+blobData.type.split('/')[1];
		self.saveCallBack && self.saveCallBack(blobData);
		//this.saveCallBack && this.saveCallBack(base64Url);
	},
	...
	setShowImg: function(){
		...
		if (this._img.width / this._img.height>=1) {
			this._img_show.width = this._option.crop_box_width;
			this._img_show.height =  this._option.crop_box_width / ( this._img.width / this._img.height );
		}else{
			this._img_show.height = this._option.crop_box_height;
