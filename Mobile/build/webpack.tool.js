...
function getEntryObject (pagesAttr) {
    let entry = {}
    pagesAttr.map(page => {
        Object.assign(entry, { [page.key]: [page.js] })
    })
    ...
    commonConfig.push({
        ...
        minChunks: function (module, count) {
            return (
                ...
                /\.js$/.test(module.resource) &&
                module.resource.indexOf(
                    path.join(__dirname, '../src/static/js/config.js')
                ) === 0
            )
        },
        chunks: entryKeys
