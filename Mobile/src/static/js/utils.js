...
const utils = {
    ...
    */
    ...
    parseDate: function (strDate, defaultValue) {
        ...
        strDate = strDate.replace('年', '-').replace('月', '-').replace('日', ' ').replace('时', ':').replace('分', '').replace(/T/i, ' ')

        var reg = /^(\d{4})([-|/])(\d{1,2})\2(\d{1,2})(\s(\d{1,2}):(\d{1,2})(:(\d{1,2}))?)?(.\d{1,3})?$/
    ...
    http: function (options) {
                ...
                if (options.hasOwnProperty(propMap[prop])) {
                    ...
                    switch (prop) {
                        ...
                        case 'data' && defaultOpts['method'].toLowerCase() === 'get':
                            ...
                            break
                        case 'url' && options['url'].indexOf('http://') === 0:
        ...
        return axios(defaultOpts)
            .then(function (response) {
                ...
                if (utils.isFunction(options.complete)) {
                    options.complete.call(context, error)
                }
