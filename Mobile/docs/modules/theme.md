### 配置vux组件颜色
开发人员可以根据系统需求更改`vux`组件的颜色，只需更改`src/static/css/theme.less`文件对应组件的颜色变量值，暂只支持文件配置变量，如下所示。
...
* tabbar
*/
@tabbar-text-active-color: #4b5c64;
...
* 实现
通过vux-loader的less-theme插件指定覆盖的less文件路径（`src/static/css/theme.less`）。
...
module.exports = vuxLoader.merge(webpackConfig, {
    plugins: ['vux-ui', 'progress-bar', 'duplicate-style', {
        name: 'less-theme',
        path: 'src/static/css/theme.less' // 指定覆盖的less
    }]
})
```
